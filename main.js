...
btnSubmit.addEventListener('click', function (e) {
    ...
    //delete
    const arrDelete = document.querySelectorAll('.icon_delete');
    // console.log(arrDelete);
    for (i = 0; i < arrDelete.length; i++) {
        arrDelete[i].onclick = (e) => {
            const postElement = e.target.parentNode.parentNode.parentNode.remove()
        }
    }

    //Edit
    const arrEdit = document.querySelectorAll(".icon_edit");
    // console.log(arrEdit);
    for (i = 0; i < arrEdit.length; i++) {
        arrEdit[i].onclick = (e) => {
            checkEdit = true;
            const postEdit = e.target.parentNode.parentNode.parentNode.querySelector("p");
